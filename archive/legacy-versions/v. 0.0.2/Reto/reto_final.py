...
def detect_yolo(frame, conf_thresh=0.55):
    ...
    if not results:
        return False, False, False, None, None, frame
    ...
    return fire_detected, anomaly_detected, shoe_detected, ax, ay, frame
...
def drone1_mission():
    ...
    - ELEVATION_SEARCH cuando memoria expira
    ...
    if t.get_battery() < 15:
        print("Bateria de dron 1 baja. Cancelando.")
        return
    ...
    elevation_search_start = 0.0
    SEARCH_MAX_TIME = 8.0  # tiempo máx de búsqueda después de elevar
    ...
    try:
        ...
        while True:
            ...
            # YOLO (fire + anomaly + shoe)
            fire_now, anomaly_now, shoe_now, ax, ay, f = detect_yolo(
                f, conf_thresh=0.45
            )
            ...
            if airborne and auto:
                ...
                # ===== FOLLOW_LINE (PID + MEMORY + ANOMALY ROI) =====
                    ...
                    if detected_line:
                        ...
                        if elapsed_mem <= MAX_MEMORY_TIME:
                            ...
                            print(">>> LOST LINE > 3s → ELEVATION_SEARCH")
                ...
                # ===== ELEVATION_SEARCH =====
                    ...
                    if not elevation_started:
                        ...
                        elevation_search_start = time.time()
                        integralcx = integralcy = integrala = 0
                        eintbx = eintby = eintba = 0
                        cxe = cye = angle_error = 0
                        last_angle = 0
                        print(">>> Elevation search: subida de 50 cm")

                    f2 = f.copy()
                    f2, angle_raw2, cx2, cy2, detected_line2 = detect_white_line_angle(f2)

                    if detected_line2:
                        print(">>> Línea encontrada después de ELEVATION_SEARCH")
                        try:
                            t.move_down(50)
                        except Exception as e:
                        ...
                        lost_line_time = time.time()
                        t.send_rc_control(0, 0, 0, 0)
                        cv2.imshow("Tello – PID + Anomaly + Memory + Shoe", f2)
                        if cv2.waitKey(1) & 0xFF == 27:
                            break
                        continue

                    search_elapsed = time.time() - elevation_search_start

                    if search_elapsed < SEARCH_MAX_TIME:
                        lr = -10
                        fb = 2
                        yaw = -15
                        t.send_rc_control(lr, fb, ud, yaw)
                        cv2.putText(f, f"SEARCHING... {search_elapsed:.1f}s",
                                    (10, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                    else:
                        if inspection_index >= MAX_INSPECTIONS:
                            print(">>> Elevation search: no line + 8 inspections → LAND")
                            t.send_rc_control(0, 0, 0, 0)
                            try:
                                t.land()
                            except Exception as e:
                            ...
                            print(">>> Elevation search: no line, inspecciones pendientes → HOLD")
                            phase = "HOLD"
                            t.send_rc_control(0, 0, 0, 0)

                # ===== HOLD (no línea, sin aterrizar) =====
                elif phase == "HOLD":
...
def second_drone_mission(fire):
    ...
    if t.get_battery() < 15:
        print("Bateria de dron 2 baja. Cancelando.")
        return
