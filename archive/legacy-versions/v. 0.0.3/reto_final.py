...
# ============================================================
model = YOLO(r"D:\carlo\Documents\Python\Concentration\Reto\v. 0.0.2\Reto\best.pt")   # nuevo best.pt con fire, anomaly, shoe

PUERTO = "COM7"  # cámbialo si tu Arduino usa otro (ej COM3, COM6...)
...
def drone1_mission():
    ...
    MAX_MEMORY_TIME = 3.0  # segundos en memoria antes de activar elevación
    ...
    # --- ELEVATION SEARCH ROBUSTO ---
    elevation_started = False
    elevation_cycle_start = 0.0
    pipe_side = None       # "LEFT", "RIGHT" o None
    side_search_start = 0.0
    SEARCH_SIDE_TIME = 3.0     # tiempo para identificar de qué lado está la tubería (arriba)
    SIDE_MEMORY_TIME = 4.0     # tiempo moviéndose en ese lado con memoria
            ...
            if airborne and auto:
                ...
                # ===== FOLLOW_LINE (PID + MEMORY + ANOMALY ROI) =====
                    ...
                    if detected_line:
                        ...
                        # 🚨 CASO ESPECIAL: YA HAY 8 INSPECCIONES → LAND directo tras 3 s sin línea
                ...
                # ===== ELEVATION_SEARCH (ROBUSTO, CON LADO Y CICLOS) =====
                    ...
                    # 2) Detección de línea para determinar de qué lado está (hasta 3 s)
                    f2 = f.copy()
                    ...
                    if pipe_side is None:
                        ...
                        if detected_line2:
                            ...
                            # No se vio bien la tubería, asumimos un lado por defecto (izquierdo)
                            pipe_side = "LEFT"
                            side_search_start = time.time()
                            print(">>> No se detectó claramente tubería al elevar, asumiendo LEFT")

                        # Mientras no se determine lado, el dron se queda quieto
                        t.send_rc_control(0, 0, 0, 0)

                    else:
                        # 3) Ya sabemos pipe_side → movimiento lateral + yaw + forward suave
                        #    y búsqueda de línea durante SIDE_MEMORY_TIME
                        f3 = f.copy()
                        f3, angle_raw3, cx3, cy3, detected_line3 = detect_white_line_angle(f3)

                        # ¿Encontramos línea en la búsqueda lateral?
                        if detected_line3:
                            print(">>> Línea encontrada después de ELEVATION_SEARCH lateral")
                            # bajar y regresar a FOLLOW_LINE
                            try:
                                t.move_down(40)
                            except Exception as e:
                                print("Error en move_down(50):", e)

                            phase = "FOLLOW_LINE"
                            lost_line_time = time.time()
                            in_memory_mode = False
                            t.send_rc_control(0, 0, 0, 0)
                            # re-sincronizar errores grosso modo
                            cxe = cx3 - cx_img
                            cye = cy3 - cy_img
                            angle_error = angle_raw3
                            continue

                        # Si aún no encontramos, seguir moviéndonos en ese lado
                        ...
                        # 4) Si se acabó SIDE_MEMORY_TIME sin encontrar línea → bajar y volver a FOLLOW_LINE
                        if elapsed_side >= SIDE_MEMORY_TIME:
                            print(">>> Side search completo sin línea → bajar y reintentar más adelante")
                            try:
                                t.move_down(40)
                            except Exception as e:
                                print("Error en move_down(50) tras side search:", e)

                            elevation_started = False
                            pipe_side = None
                            side_search_start = 0.0
                            # volvemos a FOLLOW_LINE; ahí, como no hay línea,
                            # volverá a memoria y si pasan 3 s, reentra a ELEVATION_SEARCH
                            phase = "FOLLOW_LINE"
                            lost_line_time = time.time()
                            in_memory_mode = False
                            t.send_rc_control(0, 0, 0, 0)

                # ===== HOLD (no lo usamos ahora, pero lo dejo por si acaso) =====
                elif phase == "HOLD":
                    cv2.putText(f, "PHASE: HOLD (NO LINE)", (10, 25),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    t.send_rc_control(0, 0, 0, 0)
