...
model = YOLO(r"D:\carlo\Documents\Python\Concentration\Reto\v. 0.0.5\best.pt")   # 0=FIRE, 1=ANOMALY, 2=SHOE           # ================= YOLO & ARDUINO =================
PUERTO = "COM7"
...
def detect_white_line_angle(frame):
    ...
    if ((w_box / h_box) > 0.6) and ((w_box / h_box) < 1.7):
        angle_deg = 0
        print("hola")
    return frame, angle_deg, cx, cy, True

# ================= YOLO (FIRE / ANOMALY / SHOE) =================
def detect_yolo(frame, conf_thresh=0.45):
...
def drone1_mission():
    ...
    Kicx = 0.15; Kicy = -0.023; Kian = 0.7      ## 0.4
            ...
            if airborne and auto:
                ...
                # -------- FOLLOW_LINE --------
                    ...
                    if detected_line:
                        ...
                        if current_point == 2:
                            t.send_rc_control(lr,15,ud,yaw)
                        elif current_point == 3:
                            t.send_rc_control(lr,12,ud,yaw )
                        else:
                            t.send_rc_control(lr,fb+13,ud,yaw)
                        ...
                        if elapsed_mem <= MAX_MEMORY_TIME:
                            lr = int(last_lr*0.5) - 15
                            fb = int(last_fb*0.5) + 9
                            yaw = int(last_yaw*0.5) - 23
                ...
                # -------- INSPECT --------
                    ...
                    if current_point == 2:
                        COOLDOWN_INSPECTION = 9
                        print(COOLDOWN_INSPECTION)
                    elif current_point == 3:
                        COOLDOWN_INSPECTION = 3
                        print(COOLDOWN_INSPECTION)
                    else:
                        COOLDOWN_INSPECTION = 3.7
                        print(COOLDOWN_INSPECTION)
