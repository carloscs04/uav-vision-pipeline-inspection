import time, math, io, contextlib
import cv2, numpy as np, keyboard, threading, serial
from djitellopy import Tello
from ultralytics import YOLO; from pywifi import PyWiFi, const, Profile
model = YOLO("best.pt")   # 0=FIRE, 1=ANOMALY, 2=SHOE           # ================= YOLO & ARDUINO =================
...
def detect_white_line_angle(frame):
    ...
    if ((w_box / h_box) > 0.6) and ((w_box / h_box) < 1.5):
        angle_deg = 0
        print("hola")
            ...
            if airborne and auto:
                ...
                # -------- FOLLOW_LINE --------
                    ...
                    if detected_line:
                        ...
                        if current_point == 2:
                            t.send_rc_control(lr,17,ud,yaw)
                        else:
...
def main():
    if connect_wifi("TELLO-FE193A",interface_index=1):
        ...
        drone1_mission()
        disconnect_wifi(interface_index=0)
